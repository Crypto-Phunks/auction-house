...
import * as selecors from '@/state/selectors/app-state.selector';
...
export class PhunkInfoComponent implements OnChanges {
  ...
  constructor(
    ...
    // Navigation (next/prev auction)
    this.navAuction$.pipe(
      withLatestFrom(
        this.store.select(selecors.selectAuctions),
        this.store.select(selecors.selectActiveAuction),
      ),
      tap(([nav, auctions, activeAuctions]) => {
